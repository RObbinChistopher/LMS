    ...
    <div class="p-4">
        <div class="row mb-5">
            <div class="col-md-4 col-6 header-col">
                <h3 class="heading">Child Category</h3>
            </div>
            <div class="col-md-8 col-6">
                <div class="btn-text m-0 add-new-cat" style="display: flex; justify-content: flex-end;">
                    <router-link to="/child-category-create" class="import2 add-cc hover-cc">Add &nbsp;New</router-link>
                </div>
            </div>
        </div>
        <div class="table-main">
            <div class="row p-2">
                <div class="col-md-6 col-12 left-side-filttle">
                    ...
                    <div class="status-filter flex-start gap-4">
                        <p class="active-lable mb-0" @click="fetchshowChildCategory(1)" style="cursor: pointer;">All</p>
                    </div>
            ...
            <!-- Loading Spinner -->
            <div class="spinner2 d-flex justify-content-center mb-3" v-if="childcategoryLoading">
            ...
            <div v-else>
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(items, index) in childCategory">
                                <th class='input-check-box'><input data-v-0b14badc="" type="checkbox"></th>
                                <th>{{ index + 1 }}</th>
                                ...
                                <td>
                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit data">
                                        <router-link :to="{ name: 'childCategoryEdit', params: { id: items.id } }"
                ...
                <Bootstrap5Pagination :show-disabled=true :limit="5" :data="currentPagination"
                    @pagination-change-page="fetchshowChildCategory" />
                <div v-if="childCategory && childCategory.length === 0" class="text"
                    style="text-align: center; margin: 10px 0 15px 0;">
                    No ChildCategory Found
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...mapActions('category', ['fetchshowChildCategory', 'fetchDeleteChildCategory', 'fetchChildCategorySearch']),
        CategorySearchWord() {
            if (this.searchQuery.length >= 3) {
                this.fetchChildCategorySearch({
        ...
        confirmDelete(id) {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.fetchDeleteChildCategory(id).then(() => {
                        this.$swal.fire({
                            title: "Deleted!",
                    ...
                    this.fetchshowChildCategory(1);
                }
            });
        }
    },
    computed: {
        ...mapState('category', ['childCategory', 'childcategoryLoading', 'pagination']),
        currentPagination() {
            return this.pagination ? this.pagination : "no pagination";
        },
        filterCategory() {
            return this.childCategory.filter(childCategory =>
                childCategory.name.toLowerCase().includes(this.searchQuery.toLowerCase())
            );
        }
    },
    mounted() {
