    ...
    <div class="p-4">
        <div class="row mb-5">
            <div class="col-md-4 col-6 header-col">
                <h3 class="heading">SubCategory</h3>
            </div>
            <div class="col-md-8 col-6">
                <div class="btn-text m-0 add-new-cat" style="display: flex; justify-content: flex-end;">
                    <router-link to="/subcategory-create" class="import2 add-cc hover-cc">Add &nbsp;New</router-link>
                </div>
            </div>
        </div>
        <div class="table-main">
            <div class="row p-2">
                <div class="col-md-6 col-12 left-side-filttle">
                    ...
                    <div class="status-filter flex-start gap-4">
                        <p class="active-lable mb-0" @click="fetchshowSubCategory(1)" style="cursor: pointer;">All</p>
                    </div>
            ...
            <!-- Loading Spinner -->
            <div class="spinner2 d-flex justify-content-center mb-3" v-if="subCategoryLoading">
            ...
            <div v-else>
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(items, index) in subCategory">
                                <th class='input-check-box'><input data-v-0b14badc="" type="checkbox"></th>
                                <th>{{ index + 1 }}</th>
                                ...
                                <td>
                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit data">
                                        <router-link :to="{ name: 'subcategoryEdit', params: { id: items.id } }"
                ...
                <Bootstrap5Pagination :show-disabled=true :limit="5" :data="currentPagination"
                    @pagination-change-page="fetchshowSubCategory" />
                <div v-if="subCategory && subCategory.length === 0" class="text"
...
export default {
    ...
    methods: {
        ...mapActions('category', ['fetchshowSubCategory', 'fetchDeleteSubCategory', 'fetchSubCategorySearch']),
        CategorySearchWord() {
            if (this.searchQuery.length >= 3) {
                this.fetchSubCategorySearch({
        ...
        confirmDelete(id) {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    this.fetchDeleteSubCategory(id).then(() => {
                        this.$swal.fire({
                            title: "Deleted!",
                    ...
                    this.fetchshowSubCategory(1);
                }
            });
        }
    },
    computed: {
        ...mapState('category', ['subCategory', 'subCategoryLoading', 'pagination']),
        currentPagination() {
            return this.pagination ? this.pagination : "no pagination";
        },
        filterCategory() {
            return this.subCategory.filter(subCategory =>
